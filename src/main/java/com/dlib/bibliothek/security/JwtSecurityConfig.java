 ...
 * ActiveDirectoryLdapAuthenticationProvider; import
 ...
 * JwtSecurityConfig extends WebSecurityConfigurerAdapter {
 * 
 ...
 * 
 * private String ldapDn = "ou=Thinkpalm_Users,DC=dc,DC=thinkpalm,DC=info"; //
 * Correct
 * 
 ...
 * @Bean
 * 
 * @Override public AuthenticationManager authenticationManager() { return new
 ...
 * http.cors().and().csrf().disable().authorizeRequests() // allow anonymous
 * resource requests .antMatchers(HttpMethod.GET, "/", "/v2/api-docs", //
 * swagger "/webjars", // swagger-ui webjars "/swagger-resources", // swagger-ui
 * resources "/configuration", // swagger configuration ".html", "/favicon.ico",
 * ".html", ".css", ".js")
 * .permitAll().antMatchers("/actuator").permitAll().antMatchers("/api/user").
 * permitAll()
 * .antMatchers("/api/preference").permitAll().and().authorizeRequests().
 * antMatchers("/h2-console") .permitAll().antMatchers(HttpMethod.GET,
 * "/book/images").permitAll() .antMatchers(HttpMethod.GET,
 * "/book/pdf").permitAll().anyRequest().authenticated().and()
 * .exceptionHandling().authenticationEntryPoint((request, response, e) -> {
 * response.setContentType("application/json;charset=UTF-8");
 * response.setStatus(HttpServletResponse.SC_UNAUTHORIZED); response.getWriter()
 * .write(new JSONObject().put("error", true) .put("error_code",
 * HttpStatus.UNAUTHORIZED.value()).put("message", "Unauthorized") .toString());
 ...
 *//**
	 ...
	 *//*
		 ...
		 * ActiveDirectoryLdapAuthenticationProvider(ldapDomain, ldapUrl, ldapDn);
