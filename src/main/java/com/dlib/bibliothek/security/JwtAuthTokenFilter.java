 ...
 * 
 * import com.dlib.bibliothek.service.impl.UserDetailsServiceImpl; import
 * com.dlib.bibliothek.util.JwtUtil;
 * 
 ...
 * IOException { try { String jwt = jwtUtil.getJwt(request); String username =
 * (null != jwt) ?
 ...
 * (jwtUtil.validateToken(jwt, jwtUtil.getDataFromToken(jwt), userDetails)) {
 * UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new
 * UsernamePasswordAuthenticationToken( userDetails, null,
 * userDetails.getAuthorities()); usernamePasswordAuthenticationToken
 * .setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
 * SecurityContextHolder.getContext().setAuthentication(
 * usernamePasswordAuthenticationToken); } } } catch (Exception ex) {
 * request.setAttribute("expired", ex.getMessage()); } chain.doFilter(request,
 * response); }
 * 
 * }
 */